function [S_R,S_H, Kfac, Prob_Interact] = calc_sizes_ridge_FD_2(R,A_tot,H,dont_guarantee_bigger,use_old_interactions)
%%
% Calculates the location of the new floe formed by ridging of floes
% of sizes Rt and the likelihood of interaction. This uses the new
% triangle-ridge type scheme

Hstar = H;
H_max = H(end);
H = H(1:end-1);

S_R = zeros(length(R),length(R),length(H)+1,length(H)+1);
Prob_Interact = S_R;
...
for i = 1:length(R)
    for j = 1:length(R)
        for k = 1:length(H)
            for l = 1:length(H)
                %% Determining the new floe size and interaction probability
                ...
                ridge_height = 5*h; 
                % width of ridge
                ridge_width = 2*sqrt(3)*ridge_height; 
                
                delta_d = 5*sqrt(3)*ridge_height; 
             
                % The ridge width is the minimum of the sizes
                delta_d = min([delta_d,d,s]); 
                ...
                Prob_Interact(i,j,k,l) = A_tot^2*(acz_d*acz_s)/(A_tot - acore_s - acore_d)^2;
                
                %% Area Loss
                
                % Ridge is fixed multiple of initial thickness of small floe.
                Anew = s^2 + d^2 - (3/5)*min(acz_d,acz_s);
                rnew = sqrt(Anew);
            ...
            S_R(i,j,k,end) = S_R(i,j,k,end-1); 
            S_R(i,j,end,k) = S_R(i,j,k,end-1); 
        ...
        S_R(i,j,end,end) = S_R(i,j,end-1,end-1); 
