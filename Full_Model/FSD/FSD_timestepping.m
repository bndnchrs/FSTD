...
while dt_sub > 0
    ...
    %%
    if doMech == 1
        [diff_mech,opening_mech,divopening,opening_ridge,Press] = do_timestep_mech(n,H,R,openwater,Prob_Interact_raft,Prob_Interact_ridge, ...
            S_Out_raft,S_Out_ridge,Kfac_raft,Kfac_ridge,H_raft,eps_I,eps_II,alpha_0,alpha_c,P_0,h_p);
    end
    % Difference due to thermodynamics
    if doThermo == 1
        [diff_thermo, dhdt_thermo,opening_thermo,pancake_area] = do_timestep_thermo(n,H,R,openwater,conc,Q_surf(i),dt_sub,h_p);
    end
    
    % Difference due to mechanics
