strs = {'SavedOutput/OneWeekSwell/Gen_FSD_2peak', ...
...
for i = 1
    ...
    load(loadstr,'Domainwidth','psisave','R','Lambda','bret_ampl','concsave', ...
        ...
        'year','day','Rmeanarea','Rmeannum','H','HMSAVE','HSAVE','Per');
    ...
    hold on
    if i~= 4
    plot(time/day,Rmeanarea(1:length(time))); 
    else
        plot(time(1:6:end)/day,Rmeanarea(1:6:length(time)),'o','markersize',2); 
    end
    if i == 1
    ...
    plot(time/day,concsave + SAsave)
           ...
           plot(time(1:6:end)/day,concsave(1:6:length(time)) + SAsave(1:6:length(time)),'o','markersize',2)
    end
    ...
    plot(time/day,wattensave(6,:)/Domainwidth,spec{i})
           ...
           plot(time(1:6:end)/day,wattensave(9,1:6:end)/Domainwidth,'o','markersize',2)
    end
    if i == 1
