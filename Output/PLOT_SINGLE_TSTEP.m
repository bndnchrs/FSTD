%% PLOT_TSTEP
    ...
    subplot_FD(numtypes,2,.075,time(1:i)/month,TotVol(1:i)/TotVol(1),2,1,'y')
    subplot_FD(numtypes,2,.075,time(1:i)/month,Q_surf(1:i)/300,2,1,'c')
    ...
    %ylim([.5*HSAVE(1) 2*HSAVE(1)])
        xlim([time(i)/month - 13 time(i)/month])
    ...
    if do_Thermo == 1
        subplot_FD(numtypes,2,.075,time(1:24:i)/month,squeeze(sum(sum(abs(fulldiffthermo(:,:,1:24:i)),2),1)),1,2,'g')
    end
    ...
    xlim([time(i)/month - 13 time(i)/month+1])
